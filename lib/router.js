...
var methods = require('methods'); // 返回解析器支持的 HTTP 方法的列表
...
Router.prototype.routes = Router.prototype.middleware = function () {
  ...
  var dispatch = function dispatch(ctx, next) {
    ...
    ctx.router = router; // 这个router是koa-router实例
    // 如果没有匹配到对应的路由模块, 那么就不执行下面逻辑，而是直接执行next。
 ...
 * @returns {Function}
 */
// 核心目的：根据当前请求的method处理返回结果
// 具体是：
// 1 若koa-router没有实现当前请求的method
// 1.1 若配置项throw为true
// 1.1.1 若notImplemented为函数 则throw这个函数的返回值
// 1.1.2 若没有配置notImplemented，则throw HttpError.NotImplemented
// 1.2 若没有配置throw，则设置status为501，并设置响应头的allow字段
// 2 若koa-router实现当前请求的method
// 2.1 若当前请求的method是OPTIONS，则设置status为200，body为空，并设置响应头的allow字段
// 2.2 若匹配路由支持的method不包含当前请求的method
// 2.2.1 若配置项throw为true
// 2.2.1.1 若methodNotAllowed为函数 则throw这个函数的返回值
// 2.2.1.2 若没有配置methodNotAllowed，则throw HttpError.MethodNotAllowed
// 2.2.2 若没有配置throw，则设置status为405，并设置响应头的allow字段
